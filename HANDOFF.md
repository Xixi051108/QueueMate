...
> 最后更新：2026-08-11
> 当前分支：`main`  
> 上一阶段代码基线：`f3701a0 test: add voucher redemption Playwright flow`
...
- 2026-08-11 对真实 Vue3 + Spring Boot + MySQL 执行：5 条用例全部通过，耗时 46.6 秒。
...
- 商家入驻审核、付费预约、充值、取消退款、消费码核销、用户取号、商家叫号和完成/过号已完成正式 Playwright 覆盖；管理员余额调整仍需补充正式用例。
...
1. 增加管理员余额调整 Playwright 用例。
2. 在已完成 Postman/Newman 正式执行的基础上补充 JMeter 正式结果。
3. 建立 GitHub Actions 后端、前端、接口和 UI 测试流水线。
...
最后检查前后端健康状态。当前基线应看到后端 144 个测试全部通过、Newman 45 个请求与 99 个断言全部通过且 `remainingArtifacts=0`、Playwright 入驻/预约/核销/排队 5 条用例全部通过且每轮 `remainingArtifacts=0`、Vite 生产构建成功；认证、商家入驻、地点、时段、预约、钱包、消费码、排队和统计接口均可用，普通用户、商家和管理员前端页面均可访问，并且项目中不存在 `password-strength`、`PasswordPolicy` 或 `PASSWORD_WEAK`。
