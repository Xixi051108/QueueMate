...
下一模块建议实现“固定预约时段”，不要马上扩展密码策略或进入前端。

建议顺序：

1. 明确时段创建、查询和关闭接口，核对 `docs/api_design.md` 与 `docs/requirement.md`。
2. 实现 `BookingSlot` 实体、Mapper、Service、Controller 和 DTO。
3. 实现公开地点时段查询，支持日期范围与状态筛选。
4. 实现地点所属 `MERCHANT/ADMIN` 创建和关闭时段。
5. 复用地点资源归属规则，商家只能管理自己地点的时段。
6. 校验容量必须大于 0、开始时间早于结束时间、地点必须启用预约。
7. 将重复时段唯一键冲突映射为稳定业务错误。
8. 增加权限、归属、参数、重复时段和关闭时段测试。
9. 增加 Postman 时段请求和 `knowledge/04-booking-slots.md`。
...
最后检查健康接口。当前基线应看到 27 个测试全部通过，认证与地点接口均可用，并且项目中不存在 `password-strength`、`PasswordPolicy` 或 `PASSWORD_WEAK`。
