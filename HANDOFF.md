...
- Vue Router 统一处理登录保护与 `USER/MERCHANT/ADMIN` 角色页面限制，登录后进入对应工作区。
...
- Vue3 全角色前端已完成；管理员创建地点仍需手动输入商家用户 ID，因为后端没有商家用户列表接口。
- 管理员取消预约仍需手动输入预约 ID，因为后端没有全局预约列表接口。
- 本轮已完成真实后端登录与只读页面联调，创建、预约、充值、叫号、核销和余额调整的正式端到端写操作仍需 Playwright 用例覆盖并清理测试数据。
