...
目前没有阻塞开发的故障，认证、地点、时段、免费/付费预约、钱包、消费码和退款模块可以正常构建、启动和调用。
...
- 我的预约首版返回完整列表，尚未分页。
- 消费码过期目前只有数据模型和状态，尚未接入自动定时过期任务。
- 钱包管理员全局流水和余额调整接口尚未实现。

## 5. 下一步计划

下一模块建议实现“现场排队状态机”，不要马上扩展密码策略或进入前端。

建议顺序：

1. 实现现场取号 `POST /venues/{id}/queue/tickets`。
2. 实现当前队列查询和本人号码查询。
3. 实现商家叫号、完成和过号接口。
4. 使用条件更新约束 `WAITING -> CALLED -> COMPLETED/MISSED`。
5. 商家只能操作自己地点的号码，管理员可全局操作。
6. 处理同一地点同一天排队号并发递增和唯一约束。
7. 增加状态机、权限和并发取号测试。
8. 同步 Postman、文档和学习记录。
9. 完整回归后提交并推送 GitHub。
...
最后检查健康接口。当前基线应看到 94 个测试全部通过，认证、地点、时段、预约、钱包和消费码接口均可用，并且项目中不存在 `password-strength`、`PasswordPolicy` 或 `PASSWORD_WEAK`。
