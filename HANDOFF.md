...
- 我的预约首版返回完整列表，尚未分页。
- 收费时段当前明确返回 `BOOKING_PAYMENT_REQUIRED`，必须接入钱包事务后才能开放。
- 已支付预约当前明确返回 `BOOKING_REFUND_REQUIRED`，必须接入退款事务后才能取消。

## 5. 下一步计划

下一模块建议实现“站内钱包支付与退款”，不要马上扩展密码策略或进入前端。

建议顺序：

1. 实现 `GET /wallets/my`、模拟充值和钱包流水查询。
2. 钱包余额扣减使用 `balance >= amount` 条件更新，防止并发扣成负数。
3. 收费预约在同一事务内完成名额占用、余额扣减、支付流水和预约记录创建。
4. 为支付流水增加业务幂等约束，防止同一预约重复扣款。
5. 取消已支付预约时在同一事务内执行状态更新、名额回补、余额退款和退款流水。
6. 防止重复取消和重复退款。
7. 增加余额不足、冻结钱包、支付异常回滚和退款异常回滚测试。
8. 增加并发收费预约验证，确认余额不为负、预约数不超容量、流水不重复。
9. 同步 Postman、数据库设计、API 文档和 `knowledge/06-wallet-payment.md`。
