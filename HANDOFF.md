...
- 当前只开放免费时段预约；收费时段返回 `409/BOOKING_PAYMENT_REQUIRED`，不占名额、不写预约。
...
当前刻意未实现收费预约扣款、已支付预约退款和钱包流水，这些属于下一模块。
...
- 目标表、接口和事务边界已写入设计文档；本轮只调整设计，尚未修改运行代码、`sql/schema.sql` 或本地数据库。
...
- 收费时段返回 `409/BOOKING_PAYMENT_REQUIRED`。
...
目前没有阻塞开发的故障，认证、地点、时段和免费预约模块可以正常构建、启动和调用。
...
- 我的预约首版返回完整列表，尚未分页。
- 收费时段当前明确返回 `BOOKING_PAYMENT_REQUIRED`，必须接入钱包、消费凭证事务后才能开放。
- 已支付预约当前明确返回 `BOOKING_REFUND_REQUIRED`，必须接入退款事务后才能取消。

## 5. 下一步计划

下一模块建议实现“站内钱包 + 预付消费码 + 退款”，不要马上扩展密码策略或进入前端。

建议顺序：

1. 实现 `GET /wallets/my`、模拟充值和钱包流水查询。
2. 钱包余额扣减使用 `balance >= amount` 条件更新，防止并发扣成负数。
3. 在 `sql/schema.sql` 增加 `booking_vouchers`，并为钱包流水增加 `(biz_type, biz_no, type)` 唯一幂等约束。
4. 收费预约在同一事务内完成名额占用、余额扣减、支付流水、预约记录和 `AVAILABLE` 消费凭证创建。
5. 实现 `POST /venues/{venueId}/booking-vouchers/redeem`，原子更新凭证为 `REDEEMED`、预约为 `FULFILLED`。
6. 取消未核销收费预约时在同一事务内执行状态更新、名额回补、余额退款、退款流水和消费码作废。
7. 防止重复扣款、重复生成消费码、重复取消、重复退款和重复核销。
8. 增加余额不足、冻结钱包、跨店核销、时间窗口、异常回滚和退款/核销竞态测试。
9. 增加并发收费预约与并发核销验证，确认余额不为负、预约数不超容量、流水和消费凭证不重复。
10. 同步 Postman、初始化 SQL、本地 MySQL 和 `knowledge/07-wallet-voucher-implementation.md`。
11. 完整回归后提交并推送 GitHub。
...
最后检查健康接口。当前基线应看到 72 个测试全部通过，认证、地点、时段和免费预约接口均可用，并且项目中不存在 `password-strength`、`PasswordPolicy` 或 `PASSWORD_WEAK`。
