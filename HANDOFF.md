...
```text
pnpm build: SUCCESS
Visual QA: 6 个页面 x 375px/1440px = 12 个组合
Horizontal overflow: 0
Browser runtime errors: 0
```

视觉 QA 使用符合当前后端 DTO 的临时本地响应，只用于页面渲染检查，临时脚本和进程已清理。由于本会话未取得数据库密码且后端未运行，真实 Spring Boot + MySQL 的登录、预约、充值、取消和取号联调仍需下一轮执行。
...
- Vue3 用户端第一版已完成，但商家叫号、消费码核销、繁忙统计和管理员页面尚未实现。
- 本轮前端只完成构建与浏览器模拟响应视觉检查，尚未使用真实后端执行端到端写操作。
...
下一模块建议完成 Vue3 商家/管理员业务闭环，并真实联调用户端写操作。

建议顺序：

1. 使用真实 Spring Boot + MySQL 联调登录、付费预约、取消退款、充值和用户取号。
2. 实现商家地点/时段管理、叫号、核销和繁忙统计页面。
3. 实现管理员地点管理、钱包流水和余额调整页面。
4. 将临时视觉检查转为正式 Playwright 自动化用例。
5. 接入 Postman/Newman、JMeter 和 GitHub Actions。
