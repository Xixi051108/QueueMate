...
QueueMate 不只是一个 Web CRUD 项目，而是一个围绕“生活排队、预约和模拟余额支付”场景搭建的测开作品集。它的生态链覆盖需求、架构、开发、测试、CI 和展示，适合用来讲清楚一个测开工程师如何从业务理解走到质量保障。
...
```mermaid
flowchart TD
    P["产品业务层<br/>排队 / 预约 / 模拟支付 / 角色权限"] --> R["需求文档层<br/>README / requirement.md"]
...
- 核心业务：注册登录、地点管理、时段预约、模拟余额支付、现场取号、叫号、完成、过号、繁忙统计
- 业务边界：不接地图、短信、微信登录和真实支付平台，所有数据使用本地模拟数据

这层的价值是提供足够真实的业务复杂度，让接口测试、权限测试、支付一致性测试、状态流转测试和并发测试都有明确对象。
...
- 数据库：MySQL 存储用户、地点、预约时段、预约单、钱包、钱包流水和现场号码
...
- 关键截图：用户预约、模拟支付流水、商家叫号、并发测试结果、CI 运行结果
- 面试讲解：预约防超卖、余额扣款一致性、权限隔离、非法状态流转、自动化测试分层
...
3. 我用站内钱包模拟一般支付能力，覆盖充值、扣款、退款和流水一致性。
4. 我用 JWT 和角色权限矩阵覆盖匿名用户、普通用户、商家和管理员。
5. 我把测试分为接口、性能、UI 和 CI 四层，而不是只做页面点点点。
6. 我用 JMeter 专门验证高并发预约不会超过容量，余额扣款不会变成负数。
7. 我用 Playwright 覆盖用户端和商家端最短主流程。
8. 我用 GitHub Actions 把构建和回归测试变成可持续执行的质量门禁。
