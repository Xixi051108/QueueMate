...
- `ADMIN` 可全局操作
- 普通用户可申请商家入驻，但不能自行授予商家身份
- 同一账号不能同时存在两条待审核入驻申请
- 管理员通过后账号同时拥有 `USER` 和 `MERCHANT`
- 驳回必须包含原因，申请人可修改后重新提交
- 已审核申请不能重复通过或驳回
...
- 地点不可用时，已关闭时段不可重新打开
- 用户成功预约开放且未满的时段
- 同一用户已有有效预约时，重复预约同一时段被拒绝
- 取消后重新预约同一时段成功，并生成新的预约记录
- 已满时段预约失败
- 已关闭时段预约失败
- 取消预约成功后名额回补
- 重复取消同一预约被拒绝
...
- 商家可输入或扫描消费码完成核销
- 商家登录后可查看本店排队并执行叫号
- 商家可创建/编辑地点、创建/开关预约时段并查看繁忙统计
- 管理员可查看全局钱包流水、调整用户余额并按预约 ID 取消预约
- 普通用户可完成三步商家入驻申请并查看待审核、通过或驳回状态
- 管理员可查看申请详情、通过申请或填写原因驳回
- 审核通过后用户可切换到商家端，同时仍可切回顾客端
- 375px、横屏和桌面页面不出现横向滚动，减少动画偏好生效
- 页面应对异常提示有可断言反馈
...
| API-004A | 预约 | 取消后重新预约同一时段 | 返回成功，旧记录保持 `CANCELLED`，新记录为 `BOOKED` |
...
| UI-004 | UI | 访客注册后登录 | 注册成功并进入普通用户工作区 |
| UI-005 | UI | 商家管理时段并核销 | 时段和预约状态按操作更新 |
| UI-006 | UI | 管理员调整余额 | 页面展示调整后的余额和流水 |
| API-013 | 入驻 | 用户重复提交待审核申请 | 返回 `MERCHANT_APPLICATION_PENDING` |
| API-014 | 入驻 | 管理员通过申请 | 新增商家身份并保留用户身份 |
| API-015 | 入驻 | 管理员驳回但未填写原因 | 返回 `REVIEW_NOTE_REQUIRED` |
| UI-007 | UI | 用户申请、管理员审核、身份切换 | 状态可追踪且可进入商家工作台 |
...
- `BookingServiceTest`：18 个，覆盖免费预约、付费预约、钱包扣款失败回滚、地点和时段可用性、过期、有效重复预约、取消后重新预约、满额、事务异常、本人查询、权限、取消、退款窗口和名额回补。
...
- `MerchantApplicationServiceTest`：6 个，覆盖提交、待审核防重、已是商家、通过授权、驳回原因和重复审核。
- 当前 Maven 全量共执行 132 个测试，结果为 0 失败、0 错误、0 跳过。
