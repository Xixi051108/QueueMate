...
### 11.1 `POST /test-support/postman-runs/cleanup`

用途：删除当前 Postman 全量回归创建的动态数据。该接口不是业务功能，只在 `e2e` profile 且 `TEST_SUPPORT_ENABLED=true` 时注册；默认、普通本地启动和生产配置均返回资源不存在。
...
- `runId` 只允许 8 到 32 位小写字母和数字。
- 动态用户必须同时匹配 `qm_<runId>`、固定测试昵称和固定测试手机号。
- 地点必须属于本地商家夹具，并匹配 `Postman Venue <runId>` 标记。
- 时段必须属于固定测试地点、由本地商家夹具在最近两天内创建。
- 管理员调整流水必须包含同一 `runId`；如果全量运行中断在余额恢复前，清理事务会先反向恢复净变化再删除流水。
- 任一已存在资源不满足标记规则时返回 `409/TEST_DATA_UNSAFE`，整个事务回滚。
- 响应中的 `remainingArtifacts` 必须为 `0`；Postman 收尾断言会据此验证数据库中不存在本轮标记数据。
