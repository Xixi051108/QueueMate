...
当前覆盖 4 条核心链路：注册、充值、付费预约、取消退款和消费凭证作废；用户取得消费码后由商家核销并拒绝重复核销；以及用户取号后商家叫号并完成服务或标记过号。
...
测试通过和失败都会在 `finally` 中调用受保护的清理接口。成功标准包括业务断言通过，以及每轮清理响应 `remainingArtifacts=0`。排队和核销用例使用两个独立浏览器上下文模拟普通用户和商家同时操作；核销时段按当前时间动态创建，始终位于后端允许的核销窗口内。
