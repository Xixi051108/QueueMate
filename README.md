...
当前已完成项目初始化、设计文档、MySQL 初始化、需求范围内的全部后端功能、Vue3 全角色前端业务闭环，以及付费预约退款、排队完成和排队过号三条可重复执行的 Playwright 链路。后续继续扩充测试覆盖并接入 CI。
...
- Playwright 已完成 3 条真实浏览器回归：付费预约取消退款，以及用户取号后商家叫号并完成/过号；每轮测试后 `remainingArtifacts=0`
...
Playwright 同样要求后端使用 E2E 启动项。测试会通过 API 创建本轮独立地点和所需时段，通过真实浏览器完成注册、充值、预约退款，以及用户取号、商家叫号、完成/过号断言，并在 `finally` 中调用受保护的清理端点。
...
当前用例位于 `tests/playwright/specs/paid-booking-refund.spec.js` 和 `tests/playwright/specs/queue-operator-flow.spec.js`。通过标准包括浏览器业务断言全部成功，以及每轮清理响应 `remainingArtifacts=0`；HTML 报告、截图、视频和 trace 均为忽略的本地运行产物。
...
- 需求范围内后端、Vue3 全角色前端和 3 条 Playwright 预约/排队主链路已经完成
- 继续补充 Playwright 核销、商家入驻和管理员流程，并执行 JMeter 正式回归
