...
当前已完成项目初始化、设计文档、MySQL 初始化、用户认证、地点管理与 RBAC、固定预约时段、免费/付费预约、钱包、消费码核销、取消退款和并发一致性。后续继续实现排队、统计、前端和 CI。
...
- 已有 94 个后端自动化测试
- 已提供 Postman 本地环境、认证、地点、时段、钱包和预约测试集合
...
- 后端认证、地点、预约、钱包和消费码主线已完成，下一步实现现场排队状态机
- 数据库先落核心表和初始化模拟数据
- 前端先做最短业务闭环，不追求复杂设计
