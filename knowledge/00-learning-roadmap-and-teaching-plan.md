...
| --- | --- |
| 当前阶段 | 阶段 A：建立全局地图 |
| 当前模块 | 模块 1：本地运行、网络与一次请求的完整路径（0.1 项目介绍按学生要求暂缓） |
| 下一课 | 1.3 补强验收：重新追踪登录请求并准确区分各层职责 |
| 已完成课程 | 3 / 44（已完成 0.2、1.1、1.2；0.1 按学生要求暂缓） |
| 最近学习日期 | 2026-09-15 |
| 当前薄弱点 | 区分 Axios 请求拦截器与响应拦截器；区分 Controller 参数校验、AuthService 凭据/用户状态检查和 Spring Security JWT 认证层；按请求与响应的真实时序完整追踪登录链路 |
| 下次开场 | 不重复整课讲授；先订正 1.3 的分层与拦截器时序，再使用未公布答案的新题完成补强验收，达到 80% 后进入 2.1 |
| 教学偏好 | 先完整讲授知识，练习统一放在课末；讲解过程中提前融入完成练习所需的相关知识 |
